...
type CSPAccount struct {
	...
	CSPRequestID  uint           `json:"csp_request_id" gorm:"not null;index"` // 元申請ID
	...
	// リレーション
	CSPRequest CSPRequest `json:"csp_request,omitempty" gorm:"foreignKey:CSPRequestID"`
...
type CSPAccountCreateRequest struct {
	CSPRequestID uint        `json:"csp_request_id" validate:"required"`
	Provider     CSPProvider `json:"provider" validate:"required"`
	AccountName  string      `json:"account_name" validate:"required,min=1,max=255"`
...
type CSPAccountResponse struct {
	...
	CSPRequestID uint         `json:"csp_request_id"`
	CreatedBy    uint         `json:"created_by"`
	CreatedAt    time.Time    `json:"created_at"`
	UpdatedAt    time.Time    `json:"updated_at"`
	CSPRequest   *CSPRequestResponse `json:"csp_request,omitempty"`
