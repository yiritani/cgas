...
func (h *CSPHandler) GetCSPRequests(c *gin.Context) {
	// クエリパラメータを取得
	...
	if status != "" {
		...
		requests, err = h.cspService.GetCSPRequestsByStatus(requestStatus)
		if err != nil {
		...
		requests, pagination, err = h.cspService.GetCSPRequestsByProjectIDWithPagination(uint(projectID), page, limit)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
		...
		requests, err = h.cspService.GetCSPRequestsByUserID(uint(userID))
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
		...
		// 通常のページング
		requests, pagination, err = h.cspService.GetCSPRequestsWithPagination(page, limit)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
...
func (h *CSPHandler) GetCSPRequest(c *gin.Context) {
	idStr := c.Param("id")
	...
	request, err := h.cspService.GetCSPRequestByID(uint(id))
	if err != nil {
...
func (h *CSPHandler) CreateCSPRequest(c *gin.Context) {
	...
	// ユーザーIDをコンテキストから取得（実際の実装では認証ミドルウェアから取得）
	...
	request, err := h.cspService.CreateCSPRequest(userID.(uint), &req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *CSPHandler) UpdateCSPRequest(c *gin.Context) {
	idStr := c.Param("id")
	id, err := strconv.Atoi(idStr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid ID"})
	...
	request, err := h.cspService.UpdateCSPRequest(uint(id), userID.(uint), &req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *CSPHandler) ReviewCSPRequest(c *gin.Context) {
	idStr := c.Param("id")
	id, err := strconv.Atoi(idStr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid ID"})
	...
	request, err := h.cspService.ReviewCSPRequest(uint(id), reviewerID.(uint), &req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *CSPHandler) DeleteCSPRequest(c *gin.Context) {
	idStr := c.Param("id")
	id, err := strconv.Atoi(idStr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid ID"})
	...
	err = h.cspService.DeleteCSPRequest(uint(id), userID.(uint))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
